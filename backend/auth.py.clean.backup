...
# =====================================================
# auth.py - نظام المصادقة والصلاحيات لمنظومة بن عبيد
# الإصدار 2.0 - مع دعم refresh tokens، إعادة تعيين كلمة المرور، وسجل الأمان
# =====================================================
...
JWT_ACCESS_EXPIRES = timedelta(hours=int(os.getenv('JWT_ACCESS_EXPIRES_HOURS', 24)))
JWT_REFRESH_EXPIRES = timedelta(days=int(os.getenv('JWT_REFRESH_EXPIRES_DAYS', 30)))
...
def generate_access_token(user_id: str, email: str, role: str) -> str:
    """إنشاء JWT access token قصير الصلاحية"""
    payload = {
        'user_id': user_id,
        'email': email,
        'role': role,
...
def generate_refresh_token(user_id: str) -> tuple:
    ...
    expires_at = datetime.utcnow() + JWT_REFRESH_EXPIRES
...
def _is_account_locked(email: str) -> bool:
    ...
    if attempts['count'] >= MAX_LOGIN_ATTEMPTS:
        lockout_until = attempts['first_attempt'] + timedelta(minutes=LOCKOUT_TIME_MINUTES)
...
def login():
    ...
    return jsonify({
        ...
        'user': {
            ...
            'full_name': user['full_name'],
            'role': user['role'],
            'can_view_prices': user.get('can_view_prices', False)
        }
    }), 200
...
def logout():
    ...
    if result.data:
        ...
        _revoke_refresh_token(user_id, refresh_token)
...
def logout_all():
    ...
    log_user_action(user_id, 'logout_all_devices')
    return jsonify({'message': 'تم تسجيل الخروج من جميع الأجهزة'}), 200
...
def reset_password():
    ...
    if not email:
        return jsonify({'error': 'الرمز غير صالح أو منتهي الصلاحية'}), 400
...
def change_password():
    ...
    if not is_valid_password(data['new_password']):
        return jsonify({'error': 'كلمة المرور الجديدة ضعيفة'}), 400
...
def get_current_user_from_token(token: str):
    ...
    if not payload or payload.get('type') != 'access':
        return None
    return _get_user_by_id(payload['user_id'])
